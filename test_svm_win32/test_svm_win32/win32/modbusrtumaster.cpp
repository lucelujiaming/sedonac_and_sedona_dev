...
extern "C" int rtu_master_read(int ctx_idx, int device_addr, int addr, float *buf, int len)
{
...
extern "C" int rtu_master_write(int ctx_idx, int device_addr, int addr, float *buf, int len)
{
    int result = -1;
...
extern "C" int rtu_master_add(int ctx_idx, int device_addr, int addr, int len)
{
...
extern "C" int rtu_master_open(int ctx_idx, int band, int parity, int data_bit, int stop_bit)
{
    char uart_name[32] = { 0 };
...
extern "C" int rtu_master_close(int ctx_idx)
{
    context_t *c = &context_table[ctx_idx];
    if (c->ctx_modbus == NULL || c->ctx_thread_running == 0) {
        return -1;
