  ...
  try:
    ...
    for opt, arg in opts:
      if opt in ("-s", "--stage"):
        ...
        usage()
        sys.exit(0)
    print("lujiaming -------------<" + stageDir + ">")
    if not stageDir: 
        stageDir = os.path.join(env.source, ".par")
    if not os.path.isdir(stageDir): 
        fail(stageDir + " is not a directory", True)
...
def archive():
  ...
  xplatformFilter = filter(lambda x:x.nodeName=="platformManifest", xml.dom.minidom.parse(manifest).childNodes)
  xplatformList = list(xplatformFilter)
  xplatform     = xplatformList[0]
